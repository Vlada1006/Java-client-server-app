...
public class CommandsProcessorUDP extends Thread {
    byte[] input;
    byte[] encryption_key;

    CommandsProcessorUDP(byte[] input, byte[] encryption_key) {
        this.input = input;
    ...
    public void run() {
        ...
        } catch (InvalidObjectException | NoSuchPaddingException | IllegalBlockSizeException |
                 NoSuchAlgorithmException | BadPaddingException | InvalidKeyException ignored) {
        }
    }
